...
Refactored with multi-language support.
...
def run_process():
    ...
    try:
        ...
        for i, company in enumerate(companies):
            ...
            website_content: Optional[str] = None  # Define before try block
            ...
            try:
                ...
                if website_content is None:
                    logging.warning(f"Website content not fetched for '{company.company_name}'.")
                    raise ValueError("Website fetch failed")

                # --- 5. Determine Target Language ---
                final_target_language = company.target_language  # Use value from Excel if present
                if final_target_language is None:
                    logging.info(f"No language specified in input for {company.company_name}. Detecting...")
                    try:
                        # Pass URL, content (can be None), and default language
                        final_target_language = determine_language(
                            url=company.website,
                            content=website_content,  # Pass content even if None (detector handles it)
                ...
                # Store the final language back on the object
                company.target_language = final_target_language
                logging.info(f"Using target language '{company.target_language}' for {company.company_name}.")
                # --- End Language Determination ---

                # If website fetch failed earlier, we probably can't proceed meaningfully
                if website_content is None:
                    logging.error(f"Cannot proceed further for '{company.company_name}' due to website fetch failure.")
                ...
                generated_letter: DevelopingLetter = letter_generator.generate(
                    ...
                    target_language=company.target_language  # Pass language
                )
                ...
                if "Error generating letter content" in generated_letter.body_html:
                    ...
                    logging.debug("Letter content generated successfully (with placeholders).")
